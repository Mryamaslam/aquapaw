(function(){
  ...
  // cart helpers
  function getCart(){try{return JSON.parse(localStorage.getItem("aquapaw_cart")||"[]")}catch(e){return[]}}
  function setCart(c){localStorage.setItem("aquapaw_cart",JSON.stringify(c));updateCartCount()}
  ...
  if(checkoutForm)checkoutForm.addEventListener("submit",function(e){
    e.preventDefault();
    localStorage.removeItem("aquapaw_cart");
    updateCartCount();
